...
NO_AUTO_TUNE=${NO_AUTO_TUNE:-1}    # default ON: timers stay fixed; set =0 to enable bumps
CONTAINERS=${CONTAINERS:-"lb-1 lb-2 artemis-master eznet-1 eznet-2"}
...
for TPS in $TPS_LADDER; do
  ...
  if [ "$pass" != "1" ]; then
    if [ "$NO_AUTO_TUNE" = "1" ]; then
      echo "  >>> step failed; auto-tune disabled (NO_AUTO_TUNE=1) — moving on"
    else
      echo "  >>> step failed; auto-tune NOT IMPLEMENTED in stats wrapper — stopping"
    fi
    # in either case, don't keep climbing past a failed step
    break
  fi
done
